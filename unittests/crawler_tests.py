...
from whoscored_crawler import WhoScoredCrawler


# class dont_run_TestCrawlerFunctionality(unittest.TestCase):
# 
# 
#     def setUp(self):
...
class TestWhoScoredCrawler(unittest.TestCase):
    ...
    def xtest_crawler_creation(self):
        ...
        self.assertEqual(crawler.league_link, 'http://www.whoscored.com/Regions/252/Tournaments/2/Seasons/4311/Stages/9155/Show/England-Premier-League-2014-2015')
        crawler.driver.quit()
    ...
    def xtest_finding_first_month(self):
        ...
        crawler.played_months = crawler.get_played_months()
        self.assertEqual(crawler.find_start_month(), 'Aug')
        with open(crawler._bkup_fixtures_links,'w') as tmp_f:
            tmp_f.write('.'*20)
        self.assertEqual(crawler.find_start_month(), 'Aug')
        with open(path.join(crawler._bkup_folder,'Nov.pckl'),'w') as tmp_f:
            tmp_f.write('.'*20)
        self.assertEqual(crawler.find_start_month(), 'Dec')
        os.remove(path.join(crawler._bkup_folder,'Nov.pckl'))
        with open(path.join(crawler._bkup_folder,'Apr.pckl'),'w') as tmp_f:
            tmp_f.write('.'*20)
        self.assertEqual(crawler.find_start_month(), 'May')
        os.remove(path.join(crawler._bkup_folder,'Apr.pckl'))
        with open(path.join(crawler._bkup_folder,'May.pckl'),'w') as tmp_f:
            tmp_f.write('.'*20)    
        self.assertRaises(ValueError, crawler.find_start_month)
        shutil.rmtree(crawler._bkup_folder)
        crawler.driver.quit()
